...
const message = (app) => {
    ...
    app.service(messages_shared_1.messagePath).hooks({
        ...
        before: {
            all: [
                schema_1.hooks.validateQuery(messages_schema_1.messageQueryValidator),
                schema_1.hooks.resolveQuery(messages_schema_1.messageQueryResolver)
            ],
            find: [],
            get: [],
            create: [
                schema_1.hooks.validateData(messages_schema_1.messageDataValidator),
                schema_1.hooks.resolveData(messages_schema_1.messageDataResolver)
            ],
            patch: [
                schema_1.hooks.validateData(messages_schema_1.messagePatchValidator),
                schema_1.hooks.resolveData(messages_schema_1.messagePatchResolver)
            ],
            remove: []
        },
        after: {
